...
"""
Module to handle identica_bot communication.
"""
...
IDENTICA_PATH = os.path.join('source', 'identica')
...
MESSAGES = [
	...
	{
		'en': 'Push button to choose Website',
		'ru': 'Нажмите кнопку для выбора Вебсайта'
	},
	{
	...
	{
		'en': 'Push button to confirm password:',
		'ru': 'Нажмите кнопку для подтверждения пароля:'
	}
]
KEYBOARDS = [
	[
		...
		[
			{
				'id': '/switch',
				'title': {
					'en': 'Switch to another Website',
					'ru': 'Переключиться на другой Вебсайт'
				}
			}
		],
		[
...
class IdenticaManager():
	...
	def __init__(self, debug_mode: bool = False) -> "IdenticaManager":
		"""
	...
	def init_config(self, debug_mode) -> bool:
		...
		logging.debug('IdenticaManager configuration initiated')
		return True
	...
	def run(self) -> None:
		...
		try:
			result = True
			logging.debug('IdenticaManager start main loop')
			while True:
				try:
			...
			pass
		logging.debug('IdenticaManager stop main loop')
	...
	def handle_message(self, message: dict) -> bool:
		...
		if message['text'] == '/start':
			...
			if pseudo_command in ['/select', '/switch']:
				response = self.send_callback(
					message['chat']['id'], MESSAGES[1][language],
					[self.config['website_marker']]
				)
			elif pseudo_command == '/auth_url':
				url_token = ''.join(
				...
				response = self.send_pin(
					message['chat']['id'], MESSAGES[3][language],
					pin, passwords
				)
		if not response['ok']:
			logging.error(response)
	...
	def handle_callback(self, callback: dict) -> bool:
		...
		if callback['data'] == self.config['website_marker']:
			response = requests.get(
				self.config['bot_url']['answerCallbackQuery'],
				json={
			...
			response = self.send_keyboard(
				...
				language, KEYBOARDS[1]
			)
		elif callback['data'].endswith(self.config['website_marker']):
